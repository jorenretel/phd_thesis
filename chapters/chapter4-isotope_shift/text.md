...
## Access to side-chain chemical shifts
...
The simplest solution to the first problem is to use direct ^13^C excitation. Obviously that leads to lower signal to noise compared to ^1^H-^13^C CP in a fully protonated protein due to the lower gyromagnetic ratio of ^13^C. Also the ^13^C T1 relaxation times are rather long and therefor a long recycle delay is necessary. However this can be reduced by adding a paramagnetic relaxation enhancer to the sample [@linser_sensitivity_2007][@linser_sidechain_2011]. An alternative solution could be to transfer magnetization from deuterium to carbon. Although deuterium actually has a slightly lower gyromagnetic ratio than ^13^C itself (4.11x10^7^ versus 6.73x10^7^ T^-1^s^-1^) its T1 time is very short allowing for very fast repetition of experiments. Deuterium excited experiments have been performed on SH3, ubiquitin and OmpG [@agarwal_highresolution_2009][@lalli_threedimensional_2011]. Aside from just using deuterium to transfer magnetization to carbons in these studies the deuterium double quantum frequency was measured in a indirect dimension, adding dispersion to the spectra.
