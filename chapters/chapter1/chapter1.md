...
The energy difference created by even the strongest NMR magnets is only very small compared to the thermal energy at any temperature that is not practically 0 K. Therefor the difference between the number of spins in the low and high energy state is only very small. For this reason NMR is an inherently insensitive method. A measurable signal can only be generated by the measuring millions of molecules in bulk at the same time. As will be explained later, in one way or another all difficulties with this method lead back to this fact.

#### chemical shift
The strength of NMR derives from the fact that the magnetic field perceived by a given spin is not only determined by the external field generated by the magnet. Also the local chemical environment around the spin, to be more precise the surrounding electron cloud, influences the magnetic field perceived by the spin. Therefore every nucleus in a molecule that has a different chemical environment has a different resonance frequency and therefore gives rise to a unique peak in the spectrum. This property is called chemical shift. There are typical chemical shift regions for nulcei that are part of different chemical moieties such as methyl groups or aromatic rings. Even if the same type of chemical group is present multiple times in the same molecule, which is generally the case in a large molecule such as a protein, individual peaks belonging to each one of those groups can still be separated from one another, given the resolution of the spectrum is high enough. The value for the chemical shift δ is normally not not reported in terms of Hz but in parts per million (ppm), which is defined as:

$$\delta = 10^{6}\frac{\nu-\nu_{ref}}{\nu_{ref}}$${#eq:ppm}

where ν~ref~ is the resonance frequency of a reference compount. A practical aspect of this measure is that it is independent of the magnetic field of the spectrometer so spectra recorded on different instruments can be easily compared.

By careful analysis of NMR spectra, often in combination with mass spectrometry, it has been possible for decades to determine the composition and structure of small organic molecules. For the application in structural biology, the chemical composition of the protein or nucleic acid of interest is generally already known in terms of the amino acid or nucleotide sequence. Of course we also know the chemical composition of the individual amino acids or nucleotides. The only missing information is the fold adopted in three-dimensional space and how this shape changes over time.


#### Measurement of the NMR signal

In order to detect the NMR signal, first a radio frequency pulse at the transition frequency is given. The duration and the amplitude of the pulse determines how much of the spins flip state. At a specific combination of duration and amplitude the population difference between the low and high energy





![Basic building blocks of experiments.](figures/simple_experiments.svg){#fig:simple_experiments}
...
In both liquid and solid samples, every molecule in the sample has a different spatial orientation with respect to the direction of the static field of the magnet. However in solution
