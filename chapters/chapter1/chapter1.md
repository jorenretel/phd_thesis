...
$$\nu_{IS} = \frac{\mu _{0}h}{4\pi }\frac{\gamma_{I} \gamma_{S} }{r_{IS}^{3}}(3cos^{2}\theta -1) (Hz)$${#eq:dipolar_splitting}

where μ~0~ is the permeability of the vacuum (4π x 10^-7^ H m^-1^), γ~I~ and γ~S~ are the gyromagnetic ratios of the interacting nulcei, r~IS~ is the distance between the two nuclei and θ is the angle between the vector IS and B~0~, as depicted in figure {@fig:dipolar_coupling}.

Two types of interaction have already been discussed. The Zeeman interaction
