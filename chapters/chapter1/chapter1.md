...
In both liquid and solid samples, every molecule in the sample has a different spatial orientation with respect to the direction of the static field of the magnet (unless there is a bias towards certain orientations such as in a crystal). For this reason the local the local magnetic field observed by a given nucleus changes from molecule to molecule. The chemical shift will be different for each orientation since the arrangement of the asymmetric electron cloud with respect to an observed spin and B~0~ will be different. This is called chemical shift anisotropy (CSA). Furthermore the angle θ, introduced in the last section, for a given pair of nuclei I-S is different in every molecule. For this reason the full range of values for the splitting caused by the dipolar coupling would be observed at the same time. In static solid samples, this is actually the case and the resulting spectra are rather featureless and contain little information. The reason why such spectra are not observed in solution NMR is that the fast isotropic tumbling averages out the anisotropic interactions. I.e. at NMR time scales the local magnetic field perceived by a spin is an average field over all orientations.

In solid state NMR, a similar effect can be created by magic angle spinning (MAS). The magic angle is the angle θ~magic~ for which the term $3cos^{2}\theta -1$ in equation {@eq:dipolar_splitting} becomes zero, which is at 54.7°. By spinning the sample around this angle, spin S will rotate through the dipole field created by I. The average θ over one full rotation is θ~magic~ and therefore the average perceived contribution to the magnetic field from the dipolar interaction is zero. Furthermore, MAS averages out the CSA, leaving the same isotropic chemical shift as is measured in solution NMR.


![A rotor at the magic angle.](figures/rotor.svg){#fig:rotor}


To effectively average the interactions.
